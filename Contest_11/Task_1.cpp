...
bool push_forward(subforwardlist **sfl, int d){
    subforwardlist *first_elem = new subforwardlist();
    first_elem->data = d;
    first_elem->next = *sfl;
    *sfl = first_elem;
    return 0;
...
bool push_where(subforwardlist **sfl, unsigned int where, int d){
    ...
    new_elem->data = d;
    new_elem->next = current_node->next;
...
int erase_where(subforwardlist **sfl, unsigned int where){
    ...
    int returnable_int = current_node->next->data;
    subforwardlist *tmp = current_node->next->next;
    delete current_node->next;
